...
if input("Ingresar tmax manualmente?: ") == "s":
    tmax = float(input("Ingrese el tiempo de ese máximo de amplitud: "))
    index_tmax = tiempo.tolist().index(tmax)
    print("index tmax =" + str(index_tmax))
    print(aceleración[index_tmax:])
    max = np.amax(aceleración[index_tmax:])
    print("max =" + str(max))
    index_max = aceleración[index_tmax:].tolist().index(max)+index_tmax
